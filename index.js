...
bot.command('myid', async (ctx) => {
  ...
  await ctx.reply(
    ...
    `Привет, ${firstName}! Этот номер нужен, чтобы я мог идентифицировать тебя в системе. ` +
    `Если ты администратор, отправь этот ID разработчику для настройки уведомлений.`,
    { parse_mode: 'Markdown' }
  );
});
